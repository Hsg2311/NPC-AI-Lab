...
private:
    ...
    static constexpr float SCREEN_SLOT_COLUMN_SCALE = 3.0f;  // 각개격파 차단 Squad 자동 가로 전개 배율
    static constexpr int   SCREEN_SLOT_COLUMN_COUNT = 7;    // 각개격파 차단 Squad 고정 열 수
    static constexpr float SCREEN_BLOCK_CENTER_BIAS = 0.5f; // 돌진 대상 -> 차단 대상 군집 방향 배치 비율
